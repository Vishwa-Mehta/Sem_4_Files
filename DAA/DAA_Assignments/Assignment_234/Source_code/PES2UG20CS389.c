...
// Static functions used and the question they are used in are mentioned as follows:-
// --------------------------------------------------------------------------------
// static int minTime(int V, int *time, int *s) : q8, q10
// static int dijkstra(int V, const connection_t connections[V][V], const int dest, const int src) : q10
// static int dijkstra_distance(int V, const connection_t connections[V][V], const int dest, const int src, int trip[V], int trip_order[V-1]) : q8
// static int horspool(int n, const char *pat, const char *text) : q7
// static void swap(airport_t *m, airport_t *n) : q4
// static void heapify(airport_t airport[], int n, int i, int (*predicate_func)(const airport_t *, const airport_t *)) : q4
// static void heap_sort(int n, airport_t airport[n], int (*predicate_func)(const airport_t *, const airport_t *)) : q4
// static int stringlen(const char a[]) : q5, q7
// static void stringcopy(const char a[], char b[]) : q5


static int minTime(int V, int *time, int *s) {
   int min = INT_MAX;
   int min_index;
   for (int i = 0; i < V; i++)
	   if (s[i] == 0 && time[i] <= min) {
...
static int dijkstra_distance(int V, const connection_t connections[V][V], const int dest, const int src, int trip[V], int trip_order[V-1]) {
	int dist[V];
	int s[V];
	for (int i = 0; i < V; i++) {
		dist[i] = INT_MAX;
...
static void stringcopy(const char a[], char b[]) {	
	int i;
	for (i = 0; a[i] != '\0'; i++) {
        b[i] = a[i];
    }
...
static int stringlen(const char a[]) {
	int length = 0;
	for(int i = 0; a[i]!='\0'; i++)
    {
        length++;
...
pair_t q5(int n, airport_t airports[n])
{
	...
	char s1[10];
	for(int i = 0; i < n; i++) {
		stringcopy(airports[i].airport_name, s1);
		for(int j = i+1; j < n; j++) {
			stringcopy(airports[j].airport_name, s);
			m = stringlen(airports[j].airport_name);
			int z = 0;
			while(z < m && (s[z] == s1[z]))
				z++;
			if(y < z) {
...
int q8(int n, int trip_order[n - 1], const connection_t connections[n][n])
{
	...
	cost = dijkstra_distance(n, connections, d, s, trip, trip_order) + connections[d][s].distance;
	trip[d] = 1;
	for (int i = 0; i < n-1; i++) {
		if (trip_order[i] == -1)
			trip_order[n-2] = d;
